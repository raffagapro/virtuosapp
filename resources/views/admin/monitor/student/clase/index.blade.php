                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                @forelse ($clase->homeworks->sortBy('edate', SORT_REGULAR, true) as $h)
                                    @if ($h->student === 0 || $h->student === $student->id )
                                        @php $homeworkCounter++;@endphp
                                        <tr>
                                            <td class="text-left border-right pl-5"><a href="{{ route('admin.studentMonitorTarea', [$h->id, $student->id]) }}">{{ $h->title }}</a></td>
                                            @php
                                                $edate = date('M d', strtotime($h->edate));
                                            @endphp
                                            <td class="border-right">{{ $edate }}</td>
                                            @php
                                                $foundHomework = App\Models\StudentHomework::where('homework_id', $h->id)->where('user_id', $student->id)->first();
                                            @endphp
                                            @if ($foundHomework)
                    ...
                    <a href="javascript:void(0);" type="button" id="{{ $foundChat->id }}" class="btn btn-link p-0 messageBtn @if ($clase->teacher === 0) disabled @endif" data-toggle="modal" data-target="#chatModal">
                        ...
                        @php
                            ...
                            foreach ($foundChat->chatMessages as $cmw) {
								if ($cmw->user_id !== $student->id && $cmw->status === 0) {
									$unreadMessages++;
								}
							}
                        @endphp

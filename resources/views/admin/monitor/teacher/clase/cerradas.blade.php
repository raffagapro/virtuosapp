<table class="table table-bordered">
    ...
    @endif
    <tbody>
      @php $cerradasCounter = 0;@endphp
      @forelse ($clase->homeworks->sortBy('edate', SORT_REGULAR, true) as $h)
        @if ($h->edate < $date)
          @php $cerradasCounter++;@endphp
          <tr>
            <td class="text-left"><a href="{{ route('admin.teacherMonitorTareaIndv', $h->id) }}">{{ $h->title }}</a></td>
            @php
            ...
            <td>
              ...
              action="{{ route('admin.teacherDelHomework', $h->id) }}"
              method="POST"
              style="display: none;"
              >@csrf
              @method('DELETE')
      ...
      @endforelse
      @if ($cerradasCounter === 0 && count($clase->homeworks) != 0)
        <tr>
          <td colspan="4">
            <div class="alert alert-danger" role="alert">
                Sin tareas
